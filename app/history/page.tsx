...
export default function HistoryPage() {
  ...
  const load = useCallback(async () => {
    ...
    const { data } = await supabase
      .from("trips").select("*, room:rooms(*)")
      .eq("user_id", user.id).eq("status", "ended")
  ...
  async function toggleExpand(trip: TripWithRoom) {
    ...
    if (trip.retention_tier <= 2) {
      const { data } = await supabase.from("location_points").select("*").eq("trip_id", trip.id).order("recorded_at", { ascending: true });
      setExpandedPoints(data ?? []);
      setExpandedSummary(null);
    } else {
      const { data } = await supabase.from("trip_summaries").select("*").eq("trip_id", trip.id).maybeSingle();
      setExpandedSummary(data);
      if (data?.waypoints) {
        setExpandedPoints(data.waypoints.map((w: { lat: number; lng: number }, i: number) => ({
          id: `wp-${i}`, trip_id: trip.id, lat: w.lat, lng: w.lng, accuracy: null, recorded_at: trip.started_at,
        })));
      }
    }
  ...
  const TIER_LABELS = {
    1: { label: t("tier_full"), color: "var(--go)" },
    2: { label: t("tier_sampled"), color: "var(--warn)" },
    3: { label: t("tier_summary"), color: "var(--text-muted)" },
    4: { label: t("tier_archived"), color: "var(--text-muted)" },
  } as Record<number, { label: string; color: string }>;

  const FILTERS: { key: Filter; label: string }[] = [
    { key: "all", label: t("filter_all") },
    { key: "full", label: t("filter_full") },
    { key: "summary", label: t("filter_summary") },
    { key: "archived", label: t("filter_archived") },
  ];

  const filtered = trips.filter((trip) => {
    if (filter === "all") return true;
    if (filter === "full") return trip.retention_tier <= 2;
    if (filter === "summary") return trip.retention_tier === 3;
    if (filter === "archived") return trip.retention_tier === 4;
    return true;
  });
  ...
  return (
    <div className="max-w-xl mx-auto pb-28 md:pb-8">
      <div className="px-5 pt-8 pb-6 md:px-8 md:pt-10 animate-slide-down">
        ...
        <h1 className="text-3xl font-black tracking-tight">{t("history_heading")}</h1>
      </div>

      {/* Filter pills */}
      <div className="px-5 md:px-8 mb-6 animate-slide-up" style={{ animationDelay: "80ms" }}>
        <div className="flex p-1 gap-1 rounded-xl" style={{ background: "var(--surface)" }}>
          {FILTERS.map(({ key, label }) => {
            const active = filter === key;
            return (
              <button key={key} onClick={() => setFilter(key)} className="flex-1 py-2 text-xs font-bold rounded-lg transition-all"
      ...
      <div className="px-5 md:px-8 animate-slide-up" style={{ animationDelay: "160ms" }}>
        {filtered.length === 0 ? (
          <div className="py-16 text-center rounded-2xl animate-fade-in" style={{ border: "1px dashed var(--border)", background: "var(--surface)" }}>
            ...
            <p className="text-sm font-semibold mb-1">{t("history_empty_title")}</p>
            <p className="text-xs" style={{ color: "var(--text-muted)" }}>{t("history_empty_body")}</p>
          </div>
        ) : (
          <div className="rounded-2xl overflow-hidden" style={{ background: "var(--surface)", boxShadow: "var(--shadow-card)" }}>
            {filtered.map((trip, i) => {
              ...
              return (
                <div key={trip.id} style={{ borderBottom: isLast && !isExpanded ? "none" : "1px solid var(--border-subtle)" }}>
                  <div className="flex items-center gap-4 px-5 py-4 cursor-pointer transition-all hover:bg-[var(--surface-2)] active:bg-[var(--surface-2)] animate-slide-left"
                    style={{ minHeight: 72, animationDelay: `${i * 40}ms` }}
                    onClick={() => toggleExpand(trip)}>
                    <div className="flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: "var(--surface-2)" }}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        <span className="font-bold text-sm truncate">{trip.room?.name ?? t("trip_unknown_room")}</span>
                        <span className="text-[10px] font-black uppercase tracking-wider flex-shrink-0 px-1.5 py-0.5 rounded-md"
                          style={{ background: trip.retention_tier === 1 ? "var(--go-dim)" : "var(--surface-2)", color: tierInfo.color }}>
                          {tierInfo.label}
                        </span>
                      </div>
                      <div className="flex items-center gap-3 text-xs" style={{ color: "var(--text-muted)" }}>
                        <span className="flex items-center gap-1"><Clock size={10} /> {formatDuration(trip.started_at, trip.ended_at)}</span>
                        <span className="hidden sm:flex items-center gap-1"><Navigation size={10} /> {formatDistance(trip.distance_km)}</span>
                      </div>
                    </div>
                  ...
                  {isExpanded && (
                    <div className="px-5 pb-5 animate-fade-in" style={{ borderTop: "1px solid var(--border-subtle)" }}>
                      <div className="mt-4 rounded-xl overflow-hidden" style={{ height: 260 }}>
                        {loadingPoints
                          ? <div className="h-full flex items-center justify-center" style={{ background: "var(--surface-2)" }}><Loader2 size={18} className="animate-spin" style={{ color: "var(--text-muted)" }} /></div>
                          : expandedPoints.length > 0
                            ...
                            : <div className="h-full flex flex-col items-center justify-center gap-2" style={{ background: "var(--surface-2)" }}>
                                ...
                                <p className="text-sm" style={{ color: "var(--text-muted)" }}>{t("trip_no_route")}</p>
                              </div>
                        }
                      </div>
                      {expandedSummary && (
                        <div className="mt-3 grid grid-cols-2 gap-2">
                          {[
                            { label: t("trip_duration"), value: `${expandedSummary.duration_minutes} min` },
                            { label: t("trip_distance"), value: formatDistance(expandedSummary.distance_km) },
                          ].map(({ label, value }) => (
