    ...
    <div className="min-h-screen flex flex-col" style={{ background: "var(--bg)" }}>
      ...
      <div className="flex-1 flex items-center justify-center px-5 py-10">
        <div className="w-full max-w-sm animate-slide-up" style={{ animationDelay: "80ms" }}>
          <h1 className="text-3xl font-black mb-1.5 tracking-tight">{t("login_heading")}</h1>
          <p className="text-sm mb-8" style={{ color: "var(--text-muted)" }}>{t("login_subheading")}</p>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <Link href="/auth/forgot-password" className="text-xs font-semibold transition-opacity hover:opacity-70" style={{ color: "var(--text-muted)" }}>
                  {t("login_forgot")}
                </Link>
              </div>
            ...
            <button type="submit" disabled={loading} className="w-full py-4 text-sm font-bold tracking-wide disabled:opacity-40 mt-2 rounded-xl btn-glow" style={{ background: "var(--go)", color: "#000" }}>
              {loading ? <Loader2 size={16} className="animate-spin mx-auto" /> : t("login_submit")}
            </button>
          </form>

          <p className="text-center text-sm mt-6" style={{ color: "var(--text-muted)" }}>
            {t("login_no_account")}{" "}
            <Link href="/auth/register" className="font-semibold" style={{ color: "var(--text)" }}>{t("login_sign_up")}</Link>
          </p>
        </div>
      </div>
    </div>
